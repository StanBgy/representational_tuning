    ...
    for ses in range(n_sessions):
        ...
        if valid_trials:
            ...
            if targetspace == 'nativesurface':
                ...
                # load lh
                """
                @StanBgy change#: change file extenion to hdf5
                and get_data to get_fdata (deprecated)
                """
                img_lh = h5.File(
                        os.path.join(
...
def get_labels(sub, betas_dir, nsd_dir, condition_list, labels=None, categories=None):
    ...
    return label_matrix
def get_labels1(sub, betas_dir, nsd_dir, condition_list):  # lazily renamed this
    """labels = get_labels(sub, betas_dir, nsd_dir, condition_list)
